...
### P1 — 重要
- [ ] TASK-002: OpenCodeServer 本地源码扫描（第二期，来源: 2026-04-22）
- [ ] TASK-003: 前端组件测试补充（来源: 2026-04-22）
- [ ] TASK-004: 用户管理模块 — 管理员邀请成员、角色管理、用户列表页（来源: 2026-04-23）
  - 后端路由: POST /api/auth/users（邀请）、PUT /api/auth/users/:id/role（改角色）、GET /api/auth/users（列表）
  - 前端: 用户管理页面（仅 admin 可见）
